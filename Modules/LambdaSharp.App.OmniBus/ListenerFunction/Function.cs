...
using LambdaSharp.App.OmniBus.Events.Actions;
...
namespace LambdaSharp.App.OmniBus.ListenerFunction {
    ...
    public sealed class Function : ALambdaApiGatewayFunction {
        ...
        // [Route("$default")]
        public async Task<OmniBusAck> UnrecognizedActionAsync(OmniBusAction action) {
        ...
        public async Task<OmniBusAck> SubscribeAsync(OmniBusAction action) {
        ...
        public async Task<OmniBusAck> UnsubscribeAsync(OmniBusAction action) {
            var connectionId = CurrentRequest.RequestContext.ConnectionId;
        ...
        // [Route("KeepAlive")]
        public async Task<OmniBusAck> KeepAliveAsync(OmniBusAction action) {
            LogInfo($"KeepAlive action '{action.Action ?? "<null>"}' from '{CurrentRequest.RequestContext.ConnectionId}'");
            return action.AcknowledgeOk();
        }

        private Exception Abort(OmniBusAck acknowledgeAction)
