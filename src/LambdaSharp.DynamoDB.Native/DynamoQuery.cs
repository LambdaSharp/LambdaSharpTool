...
namespace LambdaSharp.DynamoDB.Native {
    ...
    public static class DynamoQuery {
        ...
        public static IDynamoQuerySortKeyConstraint Select(string pkValue)
        ...
        public static IDynamoQuerySortKeyConstraint<TRecord> Select<TRecord>(string pkValue)
            where TRecord : class
        ...
        public static IDynamoQuerySortKeyConstraint SelectFormat(string pkValueFormat, params string[] values) {
        ...
        /// <param name="values">A string array that contains zero or more strings for the partition key format string.</param>
        /// <typeparam name="TRecord">The record type.</typeparam>
        public static IDynamoQuerySortKeyConstraint<TRecord> SelectFormat<TRecord>(string pkValueFormat, params string[] values) where TRecord : class {
            for(var i = 0; i < values.Length; ++i) {
                if(values[i] is null) {
                    throw new ArgumentException($"key[{i}] is null", nameof(values));
                }
