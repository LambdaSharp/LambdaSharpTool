...
namespace LambdaSharp.ApiGateway.Internal {
    ...
    internal class ApiGatewayInvocationTargetDirectory {
        ...
        private void AssertReturnIsNotNull(MethodInfo method, [NotNull] object? returnValue) {
            if(returnValue == null) {
                throw new ApiGatewayInvocationTargetReturnException(method, "cannot be null");
            }
        }
        ...
        public bool TryGetInvocationTarget(string key, [NotNullWhen(true)] out InvocationTargetDelegate? invocationTarget) => _mappings.TryGetValue(key, out invocationTarget);
            ...
            if(method.ReturnType == typeof(Task<APIGatewayProxyResponse>)) {
                methodAdapter = async (APIGatewayProxyRequest request) => {
                    try {
                        ...
                        AssertReturnIsNotNull(method, result);
                        return await (Task<APIGatewayProxyResponse>)result;
                ...
                methodAdapter = async (APIGatewayProxyRequest request) => {
                    try {
                        ...
                        AssertReturnIsNotNull(method, task);
                        await task;
